            ...
            <div class="bg-white p-6 rounded-lg shadow-lg">
                ...
                <div class="grid grid-cols-2 gap-2">
                    @forelse($truck->images as $image)
                        <a href="{{ Storage::url($image->path) }}" target="_blank"><img src="{{ Storage::url($image->path) }}" class="rounded-md object-cover aspect-square"></a>
                    @empty
                        <p class="col-span-2 text-sm text-gray-500">لا توجد صور.</p>
                    @endforelse
                </div>
                @if($truck->video)
                    <video controls class="mt-4 w-full rounded-md"><source src="{{ Storage::url($truck->video) }}" type="video/mp4">متصفحك لا يدعم الفيديو.</video>
                @endif
            </div>
        </div>
    </div>
