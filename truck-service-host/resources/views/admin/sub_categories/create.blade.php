<x-admin.layout>
    <x-slot name="title">إضافة تصنيف فرعي جديد</x-slot>
    <x-slot name="header">
         <div class="flex justify-between items-center">
            <span>إضافة تصنيف فرعي لـ <span class="text-indigo-600">"{{ $category->name }}"</span></span>
    ...
    <div class="bg-white p-8 rounded-lg shadow-lg max-w-2xl mx-auto">
        ...
        <form action="{{ route('admin.sub_categories.store', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div>
            ...
            <div>
                <label for="icon" class="block text-sm font-medium text-gray-700">أيقونة التصنيف الفرعي</label>
